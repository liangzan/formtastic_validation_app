...
Given /^I am at the new user form$/ do
...
Then /^I should see "([^\"]*)" validation message for "([^\"]*)"$/ do |message, field|
  xpath = %\//input[@id="#{field}"]/following-sibling::span\
  nodes = find_nodes_by_xpath(xpath)
  nodes.first.text.should == message
end
...
def find_nodes_by_xpath(path)
  $browser.elements_by_xpath(path)
end
